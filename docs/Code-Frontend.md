...
| `test` | `vitest run` | Single test run; what `make test` / `make test-frontend` (and their `./make.ps1` equivalents) invoke. There is no CI job running tests today — the only workflow in `.github/workflows/` is `jekyll-gh-pages.yml` |
