...
This page documents the seven modules that make up the core of the Recluse serving layer: the package marker, settings, the FastAPI application factory, the database layer, the ORM models, the wire contracts, and the model bundle loader. It is written for anyone modifying the backend, reviewing a change against it, or trying to work out why the API reports `model_version: "unloaded"`. It reflects the repository at the end of Phase 0 — the scaffold is complete and runnable, no model has been trained, and the scoring path is an explicit stub. For the request-handling layer built on top of these modules see [Code Reference — Backend Routes](Code-Backend-Routes.md); for the offline training side see [Code Reference — Backend Training](Code-Backend-Training.md).
...
- Status: **implemented.** Every non-health endpoint reachable through `api_router` currently answers HTTP 501 with a machine-readable body — see [Code Reference — Backend Routes](Code-Backend-Routes.md) and [API Reference](API-Reference.md).
...
- The guard is regression-tested: `backend/tests/test_api_surface.py::test_startup_refuses_a_bundle_with_an_inconsistent_schema_hash` writes a bundle whose `schema_hash` does not match its `feature_order` and asserts `load_bundle()` raises. It was written in Phase 0, before there was anything to load, because the failure it prevents raises nothing on its own — see [Code Reference — Backend Tests](Code-Backend-Tests.md).
...
- [Architecture](Architecture.md) — how these modules fit the two-stage detection design
- [Configuration](Configuration.md) — the operator-facing view of every `IDS_` variable
- [Database Schema](Database-Schema.md) — the tables in `models.py` as a schema document
- [API Reference](API-Reference.md) — the full v1 surface, including what currently answers 501
- [ML Models](ML-Models.md) — what Stage 1 and Stage 2 are and how thresholds are chosen
- [Code Reference — Backend Routes](Code-Backend-Routes.md) — the handlers mounted under `api_router`
- [Code Reference — Backend Training](Code-Backend-Training.md) — `training.features` and the artifact writers
- [Code Reference — Backend Tests](Code-Backend-Tests.md) — the tests that pin the health contract, the FP-budget arithmetic and the schema-hash guard
- [Repository Layout](Repository-Layout.md) — where everything else lives
- [FAQ](FAQ.md) — why `model_version` reads `unloaded`, and other first-run questions
