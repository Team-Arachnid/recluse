...
auditing a claim made elsewhere in the wiki against reality.
...
- `make seed` populating a demo database so the dashboard is never empty on
  ...
  `typecheck`, `gen-types`, `build`, `up`, `down`, `logs`, `ps`, `wiki`,
  `wiki-check`, `hooks`, `hooks-uninstall` and `clean`, and Phase 8 adds `seed`
  to both.
